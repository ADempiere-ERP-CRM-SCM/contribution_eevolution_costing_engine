...
public class AverageInvoiceCostingMethod extends AbstractCostingMethod implements ICostingMethod {
	...
	{	
		...
		m_last_costdetail =  MCostDetail.getLastTransaction(m_model,m_trx, m_as.getC_AcctSchema_ID(),
				m_dimension.getM_CostType_ID(), m_dimension.getM_CostElement_ID() , m_model.getDateAcct(), costingLevel);
		...
		//The cost detail was created before and is necessary to update cost by generate adjustment
		...
		{		
				...
				m_CumulatedQty = getNewCumulatedQty(m_last_costdetail); // Quantity Cumulated from last transaction
				//Cost Adjustment  
	...
	private void createCostDetail()
	{
		...
		//Ignore reversal transaction because is created based on the original transaction
		if(m_model.getReversalLine_ID() > 0 && m_costdetail == null)
		{	
		...
		//if (m_costdetail != null && m_last_costdetail != null && m_costdetail.get_ID() ==  m_last_costdetail.get_ID())
		//	return ;
		
		int seqNo = m_last_costdetail.getSeqNo() + 10;
		//create a new cost detail or is necessary create a new cost detail for adjustment
		if(m_trx.getM_Transaction_ID() != m_last_costdetail.getM_Transaction_ID() && m_costdetail == null || m_AdjustCost.add(m_AdjustCostLL).signum() != 0 && m_costdetail == null)
		{	
			...
			{	
				String description = m_costdetail.getDescription() != null ? m_costdetail.getDescription() : "";
				...
				{	
					...
					m_costdetail.setAmt(m_costdetail.getCostAmt().add(m_costdetail.getCostAdjustment()));
					m_costdetail.setDescription(description + " Adjust Cost:"+ m_AdjustCost);
				}	
				//update adjustment cost lower level
				if (m_AdjustCostLL.signum() != 0)
				{	
					description = m_costdetail.getDescription() != null ? m_costdetail.getDescription() : "";
					m_costdetail.setCostAdjustmentDateLL(m_model.getDateAcct());
					m_costdetail.setCostAdjustmentLL(m_AdjustCostLL);
					m_costdetail.setCostAmtLL(BigDecimal.ZERO);
					m_costdetail.setAmt(m_costdetail.getCostAmtLL().add(m_costdetail.getCostAdjustmentLL()));
					m_costdetail.setDescription(description +" Adjust Cost LL:"+ m_AdjustCost);
				}	
			}
	...
	public void createCostAdjustment()
	{
		...
		//get the cost details that need be re process before this cost transaction
		List<MCostDetail> cds = MCostDetail.getAfterDate(m_costdetail,costingLevel);
		if(cds == null || cds.size() == 0)
			return;
		MCostDetail last_cd = m_costdetail;
		m_costdetail = null;
		/*System.out.println("-----------------------------------ADJUSTMENT COST -------------------------------------------------");
		System.out.println(last_cd);
		System.out.println("----------------------------------------------------------------------------------------------------");*/
	...
	private void updateAmtCost()
	{
		...
		{	
			...
			m_costdetail.setCostAmtLL(m_costdetail.getAmtLL().subtract(m_costdetail.getCostAdjustmentLL()));
		}	
		if(m_trx.getMovementType().contains("-"))
		{	
	...
	public void adjustCostDetail(MCostDetail cd)
	{
		...
		CostEngineFactory.getCostEngine(cd.getAD_Client_ID()).createCostDetail(m_as, trx, trx.getDocumentLine(), ct, ce);
		...
		if (MCostElement.COSTELEMENTTYPE_Material.equals(ce.getCostElementType()))
		{
    		...
    		if(MTransaction.MOVEMENTTYPE_VendorReceipts.equals(trx.getMovementType()) && MCostElement.COSTELEMENTTYPE_Material.equals(ce.getCostElementType()))
    		{
    			...
    			MMatchPO[] orderMatches = MMatchPO.getOrderLine(cd.getCtx(), line.getC_OrderLine_ID(), cd.get_TrxName());
    			for(MMatchPO match: orderMatches)
    			{	
					if(match.getM_InOutLine_ID() == line.getM_InOutLine_ID() && match.getM_Product_ID() == trx.getM_Product_ID())
					{	
						CostEngineFactory.getCostEngine(cd.getAD_Client_ID()).createCostDetail(m_as, trx, match , ct, ce );
					}
    			}
			
				MMatchInv[] invoiceMatches = MMatchInv.getInOutLine(cd.getCtx(), line.getM_InOutLine_ID(), cd.get_TrxName());
		...
		{			    			
			...
			{
    			MInOutLine line = (MInOutLine) trx.getDocumentLine();
    			for (MLandedCostAllocation allocation : MLandedCostAllocation.getOfInOuline(line,ce.getM_CostElement_ID()))
    			{	
    				CostEngineFactory.getCostEngine(cd.getAD_Client_ID()).createCostDetailForLandedCostAllocation(allocation);
    			}				    			
			}
			return;
    	}
